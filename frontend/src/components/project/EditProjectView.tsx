...
const EditProjectView: React.FC<EditProjectViewProps> = ({
	projectId,
}) => {
	...
	const [pdfFilesToSave, setPdfFilesToSave] = useState<
		File[]
	>([]); // new pdfs to save
	const [deleteDocuments, setDeleteDocuments] = useState<
		string[]
	>([]); // ids of pdfs to delete
	...
	const getCurrentNodeDataFromCanvas = useCallback(
		(currentNodes: Node[]) => {
	...
	const createWorkflowNodes = useCallback(
		(
			...
			allNodeTypes.forEach((nodeType, index) => {
					...
					data: {
						...
						onFilesToDeleteChange:
							nodeType === "knowledgeBaseNode"
								? onFilesToDeleteChange
								: undefined,
						label: initialSidebarNodes.find(
							(n) => n.id === nodeType
						)?.label,
				...
				if (index > 0) {
					const sourceNodeId = `${allNodeTypes[index - 1]}-${
						index - 1
					}`;
	...
	const isNodeInConnectedPath = useCallback(
		(
			...
			const pathFromUserToNode = findPathToOutput(
				userQueryNode.id,
				nodeId
			);
			const pathFromNodeToOutput = findPathToOutput(
				nodeId,
				outputNode.id
			);
	...
	const updateDraftConfigWithConnectedNodes = useCallback(
		(currentNodes: Node[], currentEdges: Edge[]) => {
			setDraftConfig((prevConfig) => {
				...
				const workflowTypes = newWorkflow
					? newWorkflow.split("_")
					: [];
		...
		[
			calculateWorkflowFromConnectedNodes,
			isNodeInConnectedPath,
		]
	);
	...
	const onFilesToDeleteChange = useCallback(
		(fileId: string) => {
	...
	const onBasicInfoChange = useCallback(
	...
	useEffect(() => {
		if (currentProject && currentProject.workflow) {
				...
				createWorkflowNodes(
					{
						...
						knowledge_base_node:
							currentProject.knowledge_base_node,
			...
			setSidebarNodes(
				initialSidebarNodes.filter(
					(n) => !nodesOnCanvas.has(n.id)
				)
			);
		} else {
			setNodes([]);
		...
		setProjectConfig({
			...
			web_search_node: {
				serpapi_api_key:
					currentProject?.web_search_node?.serpapi_api_key,
			},
		});
		setDraftConfig({
			workflow: currentProject?.workflow,
	...
	const onDeleteNode = useCallback(
		(nodeId: string) => {
			setNodes((prevNodes) => {
				const nodeToDelete = prevNodes.find(
					(n) => n.id === nodeId
				);
				...
				if (sidebarNode) {
					setSidebarNodes((currentSidebarNodes) => {
						if (
							!currentSidebarNodes.some(
								(s) => s.id === sidebarNode.id
							)
						) {
	...
	const onConnect = useCallback(
		(connection: Connection) => {
			...
			setEdges((prevEdges) => {
				...
				updateDraftConfigWithConnectedNodes(
					nodes,
					updatedEdges
				);
				return updatedEdges;
		...
		(event: React.DragEvent) => {
			...
			const newNode: Node = {
				...
				data: {
					label,
					pdf_files:
						type === "knowledgeBaseNode" ? pdfFiles : undefined,
					onDelete: onDeleteNode,
					files_to_upload:
						type === "knowledgeBaseNode" ? pdfFilesToSave : [],
					onDataChange: onNodeDataChange,
					onFileUpload:
