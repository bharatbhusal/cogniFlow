...
export const queryProject = createAsyncThunk(
	...
	async (
		...
		try {
			const response = await apiClient.queryProject(
				projectId,
				queryData
			);
			return response.data;
						...
						state.currentProject = {
							...
							llm_node:
								updates.llm_node.status !== "unchanged"
									? {
											llm_model_name:
												updates.llm_node.new_llm_model_name,
											openai_api_key:
												updates.llm_node.new_openai_api_key,
									  }
							...
							knowledge_base_node:
								updates.kb_node.status !== "unchanged"
									? {
											openai_api_key:
												updates.kb_node.new_openai_api_key,
